...
esp_err_t esp_lcd_touch_new_i2c_gsl3680(esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *out_touch)
{
    ...
    // Skip firmware verification - some chips don't support 0xb0 readback
    // Just assume firmware loaded OK since we got here without I2C errors
    ret = ESP_OK;
    ESP_LOGI(TAG, "GSL3680 initialization complete (skipping firmware verification)");
...
// #endif
    ...
    // Log errors only
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "❌ I2C read error: %s (0x%x)", esp_err_to_name(err), err);
    }
...
static bool esp_lcd_touch_gsl3680_get_xy(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
    ...
    *point_num = Finger_num;
    ...
    // Copy touch data for all detected fingers
    if(Finger_num > 0)
    {
        uint8_t count = (Finger_num < max_point_num) ? Finger_num : max_point_num;
...
static esp_err_t esp_lcd_touch_gsl3680_read_ram_fw(esp_lcd_touch_handle_t tp)
{
    ...
    vTaskDelay(pdMS_TO_TICKS(200));  // Increased from 30ms to 200ms for chip startup
    ESP_RETURN_ON_ERROR(touch_gsl3680_i2c_read(tp, addr, (uint8_t *)&read_buf, 4), TAG, "gsl3680 read error!");
    ESP_LOGI(TAG,"gsl3680 startup_chip status: read 0xb0 = 0x%02x,0x%02x,0x%02x,0x%02x (expect 0x5a,0x5a,0x5a,0x5a)",read_buf[3],read_buf[2],read_buf[1],read_buf[0]);
    if(read_buf[3] != 0x5a || read_buf[2] != 0x5a || read_buf[1] != 0x5a || read_buf[0] != 0x5a)
    {
        ESP_LOGW(TAG, "GSL3680 chip not ready - may not respond to touch");
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "✓ GSL3680 chip startup verified successfully");
    return ESP_OK;
}
