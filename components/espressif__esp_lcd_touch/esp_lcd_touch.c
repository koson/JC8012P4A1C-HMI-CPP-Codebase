...
bool esp_lcd_touch_get_coordinates(esp_lcd_touch_handle_t tp, uint16_t *x, uint16_t *y, uint16_t *strength, uint8_t *point_num, uint8_t max_point_num)
{
    ...
    // Debug: Check transformation conditions
    static bool logged_once = false;
    if (!logged_once && *point_num > 0) {
        ESP_LOGI(TAG, "🔧 TRANSFORM CHECK: sw_adj_needed=%d point_num=%d", sw_adj_needed, *point_num);
        ESP_LOGI(TAG, "   Flags: swap_xy=%d mirror_x=%d mirror_y=%d",
                 tp->config.flags.swap_xy, tp->config.flags.mirror_x, tp->config.flags.mirror_y);
        ESP_LOGI(TAG, "   Callbacks: set_swap_xy=%p set_mirror_x=%p set_mirror_y=%p",
                 tp->set_swap_xy, tp->set_mirror_x, tp->set_mirror_y);
        ESP_LOGI(TAG, "   x_max=%d y_max=%d", tp->config.x_max, tp->config.y_max);
        logged_once = true;
    }
    ...
    for (int i = 0; (sw_adj_needed && i < *point_num); i++) {
        ...
        if (raw_x != x[i] || raw_y != y[i]) {
            ESP_LOGI(TAG, "🔄 TRANSFORM: Raw(%d,%d) → Final(%d,%d) | swap=%d mirror_x=%d mirror_y=%d x_max=%d y_max=%d",
                     raw_x, raw_y, x[i], y[i],
                     tp->config.flags.swap_xy, tp->config.flags.mirror_x, tp->config.flags.mirror_y,
