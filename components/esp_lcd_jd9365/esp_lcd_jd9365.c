 ...
 */
 ...
 #define JD9365_CMD_MV_BIT       (1 << 5)  // Column/Row Exchange (swap X/Y)
 ...
 static esp_err_t panel_jd9365_swap_xy(esp_lcd_panel_t *panel, bool swap_axes)
 {
     ...
     // Control column/row exchange (swap X/Y) through MADCTL MV bit
     if (swap_axes) {
         madctl_val |= JD9365_CMD_MV_BIT;
     } else {
         madctl_val &= ~JD9365_CMD_MV_BIT;
     }
     ...
     ESP_LOGI(TAG, "swap_xy: %s, MADCTL=0x%02X", swap_axes ? "true" : "false", madctl_val);
