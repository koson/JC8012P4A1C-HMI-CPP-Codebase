...
#include "esp_lcd_touch_gsl3680.h"  // ✅ Using custom GSL3680 driver from components/
...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /* Initialize touch */
    static esp_lcd_touch_io_gsl3680_config_t gsl3680_io_cfg = {
        .dev_addr = ESP_LCD_TOUCH_IO_I2C_GSL3680_ADDRESS,
    };
    
    const esp_lcd_touch_config_t tp_cfg = {
        .x_max = BSP_LCD_H_RES,  // 800 - touch native resolution
        .y_max = BSP_LCD_V_RES,  // 1280 - touch native resolution  
        .rst_gpio_num = BSP_LCD_TOUCH_RST, // GPIO_NUM_22
        .int_gpio_num = GPIO_NUM_NC, // ⚠️ TEMP FIX: Disable interrupt, use POLLING mode instead of EVENT mode
        .levels = {
            .reset = 0,
            .interrupt = 0,
        },
        .flags = {
            .swap_xy = 0,  // ❌ ปิด! ให้ LVGL จัดการ rotation
#if CONFIG_BSP_LCD_TYPE_1024_600
            .mirror_x = 1,
            .mirror_y = 1,
#else
    ...
    /* Using custom GSL3680 driver from components/ (not managed_components) */
...
static lv_indev_t *bsp_display_indev_init(lv_display_t *disp)
{
    ...
    // ⚠️ Allow touchscreen init to fail gracefully (system works without touch)
    esp_err_t ret = bsp_touch_new(NULL, &tp);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "⚠️ Touchscreen initialization failed (err=%s) - continuing without touch input", esp_err_to_name(ret));
        ESP_LOGW(TAG, "⚠️ Display will work but touch events will not be available");
        return NULL;  // System continues without touch
    }
    
    if (tp == NULL) {
        ESP_LOGW(TAG, "⚠️ Touchscreen handle is NULL - continuing without touch input");
        return NULL;
    }
...
lv_display_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    // ⚠️ Allow touchscreen init to fail gracefully - display works without touch
    disp_indev = bsp_display_indev_init(disp);
    if (disp_indev == NULL) {
        ESP_LOGW(TAG, "⚠️ Touch input not available - display will work in view-only mode");
    } else {
